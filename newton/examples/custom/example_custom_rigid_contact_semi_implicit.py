...
class Example:
    def __init__(self, viewer):
        ...
        body_sphere = builder.add_body(
            ...
            mass=5.0  # Body mass = 1.0 kg
        )
        
        # Create shape config with zero density (shape adds no mass)
        sphere_cfg = builder.default_shape_cfg.copy()
        sphere_cfg.density = 1.0  # Shape contributes no mass
        
        builder.add_shape_sphere(
            body=body_sphere,
            radius=0.1,  # 10cm radius
            cfg=sphere_cfg,  # Use config with density=0
        )
