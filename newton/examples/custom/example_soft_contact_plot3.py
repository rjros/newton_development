...
class Example:
    ...
    def log_forces(self):
        ...
        # NEGATIVE value = cube expanded (shouldn't happen in compression test)
...
Total: {sphere_force[-1] + cube_force[-1]:.2f} N
        ...
        # --- Plot 3: Force-Displacement Curve ---
        ...
        ax3.set_title('Force-Displacement Curve (colored by time)', fontsize=14, fontweight='bold')
        ...
        if len(time) > 10:
            # Draw arrow from early to late time
            early_idx = len(time) // 4
            late_idx = len(time) // 2
            ax3.annotate('', 
                        xy=(top_displacement[late_idx], sphere_force[late_idx]),
                        xytext=(top_displacement[early_idx], sphere_force[early_idx]),
                        arrowprops=dict(arrowstyle='->', color='red', lw=2))
            ax3.text(top_displacement[early_idx], sphere_force[early_idx], 
                    '  Impact', fontsize=9, color='red', fontweight='bold')
        
        # Overall title
        fig.suptitle(f'Force Analysis - {self.model.particle_count} Particles ({self.cell_dim}×{self.cell_dim}×{self.cell_dim} Grid)', 
        ...
        print(f"  Logged {len(time)} frames")
