...
class Example:
    def __init__(self, viewer, verbose=False, cell_dim=3):
        ...
        body_sphere = builder.add_body(
            xform=wp.transform(p=wp.vec3(0.0, 0.42, sphere_height), q=wp.quat_identity()),
            mass=sphere_mass
        )
...
R = {R*100:.0f} cm
        ...
        # Overall title
        fig.suptitle(f'Force Analysis with Hertzian Contact Theory - {self.model.particle_count} Particles ({self.cell_dim}×{self.cell_dim}×{self.cell_dim} Grid)', 
